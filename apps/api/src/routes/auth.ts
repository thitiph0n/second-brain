...
auth.get("/github/callback", authRateLimit, async (c) => {
	try {
		...
		const oauthState = await oauthService.validateOAuthState(c.env.CACHE, state);
		if (!oauthState) {
			return c.redirect(`${c.env.FRONTEND_URL}/auth/error?error=Invalid+or+expired+state`);
		}
		...
		const { accessToken: jwtToken, refreshToken } = await authService.createSession(user);
